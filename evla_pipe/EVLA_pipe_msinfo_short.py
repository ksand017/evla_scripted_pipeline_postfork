...
polcals_C = ['J0542+4951', '3c147', '3C147', 'J1407+2827', 'OQ208', 'Oq208', 'oq208', 'J2355+4950'] # 'J0259+0747'
use_parang = False
if len(polarization_angle_state_IDs) == 0:
    if do_pol == True:
        ...
        for i in range(len(polcals_A)): #check which std cat A polcal sources are present in MS 
            ...
            for i in range(len(polcals_C)):
                if polcals_C[i] in field_names:
                    if has_leak_polcal == False:
                        polLeakField = "%s" % polcals_C[i]
                    else:
                        polLeakField += ",%s" % polcals_C[i]
                    has_leak_polcal = True
                    leak_polcal_type.append('Df')
                    ...
                    #break #identify ALL possible calibrators
